                    ...
                    foreach ($invoiceItems as $item) {
                        ...
                        if (!in_array($item['invoice']['customer'], $paidCustomers)) {
                            ...
                            if (in_array($checkin['person']['@id'], $customers)) {
                                foreach ($variables['checkins'] as $k => $check) {
                                    if ($check['person']['@id'] == $checkin['person']['@id']) {
                                        $variables['checkins'][$k] = $checkin;
                                    }
                                }
                            } else {
                                $variables['checkins'][] = $checkin;
                            }
                            if ($checkin['paymentStatus'] == 'paid') {
                                $paidCustomers[] = $checkin['person']['@id'];
                            }
                            $customers[] = $checkin['person']['@id'];
                        }
                    }
                }
            }
...
//        $person = 'https://dev.larping.eu/api/v1/cc/people/5c74f378-a85f-4415-8f76-e702f2d727dd';
        ...
        $checkins = $commonGroundService->getResourceList(['component' => 'chin', 'type' => 'checkins'], ['person' => $person, 'node.event' => $event['@id']])['hydra:member'];
...
//                }
            ...
            return $this->redirect($this->generateUrl('app_dashboardorganization_members'));
        } elseif ($request->isMethod('POST') && $request->get('mailUser')) {
            // Get app_id of larping application
            $providers = $commonGroundService->getResourceList(['component' => 'uc', 'type' => 'providers'], ['type' => 'id-vault', 'application' => $params->get('app_id')])['hydra:member'];
            $appId = $providers[0]['configuration']['app_id'];
            ...
            $data = [];
            $data['description'] = $request->get('description');
            $data['organization'] = $variables['organization']['name'];
